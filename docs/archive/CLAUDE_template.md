...
| `## Standing Rules` section of this file | Read only |
| Everything else in this file | Read + Write |
| `shared/SHARED_FEEDBACK.md` | Append only |
| `docs/wip/*.md` | Read + Write |
| `docs/archive/` | Write — CC may move superseded docs here |
| `docs/TECH_STACK.md` | Read + Write |
| `docs/ARCHITECTURE.md` | Read + Write |
| `docs/SITE_DESIGN.md` | Read + Write |
| `docs/CONTENT.md` | Read + Write |
...
- When the feature ships, notify the user — the wip file moves to `docs/archive/`
  as part of transition cleanup
...
4. After a successful transition: move files to their proper locations, move
   superseded docs to `docs/archive/`, clear `incoming/`, update `## Current State`
